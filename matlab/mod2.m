...
if doSineInput
    ...
    OUT = abs(fft(windo.*out./window_amp_correct));
...
if doSineInput  
    fname = sprintf("../verilog/QNS/sim/test_mod2_sine_n19r15_fin_1k_0p5VFS_OSR_%d_fs_44p1k_%d_pts.txt",OSR,length(u));
else
    fname = sprintf("../verilog/QNS/sim/test_mod2_rand_7283723_n19r15_OSR_%d_fs_44p1k_%d_pts.txt",OSR,length(u));
end
verilogData = readmatrix(fname);
...
verilog_out_filt = conv(verilogModulatorOut, AAF);
...
title(sprintf("rms error b/t verilog and matlab models: %.4E",root_mse))
xlabel('sample number')
ylabel('error (dB)')
subplot(212)
plot(verilog_out_normfs, '*'); hold on
plot(u_normfs, '--')
xlim([length(u_normfs)/2-50, length(u_normfs)/2+50, ])
title('decimated output vs decimated input')
xlabel('sample number')
